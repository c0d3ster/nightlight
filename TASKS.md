...
- [ ] #12 [stack: solo] **Add turn/cache-read efficiency guidance to CLAUDE.md's Overnight Agent Workflow**
  Cost-audited the 2026-08-06 c0d3ster session's raw stream-json in detail (200 turns, $16.61 for 2 tasks) and found four concrete, avoidable turn-count drivers beyond the cross-task carryover task #2 already targeted: (1) only 21/200 turns batched independent tool calls into one turn — `Edit` (31% of session cost) and `Read` (18%) are the two biggest categories and both are dominated by one-call-per-turn issuance; (2) 52 of 66 `Edit` turns were repeat edits to only 15 already-touched files — `MultiEdit` would collapse most of these into one turn; (3) verification commands (typecheck/lint/test) were issued as separate `Bash` turns instead of chained in one invocation; (4) investigation reads skewed toward full-file reads where a targeted grep/glob would have answered the question. Each of these pays a full cache-read of the accumulated conversation per turn, so cutting turn count cuts cost directly.

  Acceptance criteria: CLAUDE.md's Overnight Agent Workflow section gets a new subsection (e.g. "Turn efficiency") documenting: batch independent read-only tool calls (`Read`/`Glob`/`Grep`) into a single turn when their inputs don't depend on each other; use `MultiEdit` instead of sequential `Edit` calls when making 2+ edits to the same file; chain verification commands (typecheck/lint/test) into one `Bash` invocation via `&&` where the target repo's tooling supports it; prefer targeted grep/glob over full-file reads during investigation unless the whole file's content is actually needed. Applies across all target repos — this lives in the shared Overnight Agent Workflow section, not `overnight.sh` itself.
