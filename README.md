...
Two files land in `logs/`, both gitignored: `<repo>-<date>.jsonl` (the raw NDJSON event stream — unformatted, kept for later inspection or tooling) and `<repo>-<date>.stderr.log` (anything the `claude` process wrote to stderr, kept separate so it doesn't break the JSON stream). The formatted view in your terminal isn't saved anywhere by default; redirect it yourself (`./overnight.sh some-repo | tee logs/some-repo-readable.log`) if you want a saved copy of the human-readable version too.
